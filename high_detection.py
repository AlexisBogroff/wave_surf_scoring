...
class image_vague:
    ...
    def dessiner_lignes_vertes_2(self,step=10,threshold_min=0,threshold_max=100):
        ...
        for stepX in range(step, 40, 5):
          for threshold_high in range(threshold_min+10,threshold_max, 10):
            for threshold_low in range(threshold_min, threshold_high, 10):
              #reinitialisatio des variables
              output_img = self.image_box.copy()
              ...
              #debut du teste apres avoir fixé step et threshold
              for y in range(self.y, 100, -step):
                ...
                line_percentage = np.count_nonzero(sub_rect) / (sub_rect.shape[0] * sub_rect.shape[1])
              ...
              if self.lines_above_surfer <= 2 and self.lines_above_surfer >=1 :
                break
            if self.lines_above_surfer <= 2 and self.lines_above_surfer >=1 :
              break
          if self.lines_above_surfer <= 2 and self.lines_above_surfer >=1:
    ...
    def detect_lines(self, binary_image, step, stepX, threshold_low, threshold_high):
        ...
        for y in range(self.y, 100, -step):
            ...
            x_start = max(0, int(self.x - 50))  # 50 pixels to the left of the surfer's center
            x_end = min(binary_image.shape[1], int(self.x + 50))  # 50 pixels to the right of the surfer's center
    ...
    def dessiner_lignes_vertes_3(self, step=10, threshold_min=0, threshold_max=100):
        for stepX in range(step, 40, 5):
            for threshold_high in range(threshold_min + 10, threshold_max, 10):
                for threshold_low in range(threshold_min, threshold_high, 10):
...
i=0
for i in range(3):
    Vague.append(image_vague("vague"+str(i)+".jpg"))
    Vague[i].Taille_surfeur()
    i=i+1
...
surf=image_vague("vague1.jpg")
surf.Taille_surfeur(net)
#surf.affichage_image("vague1",surf.image_box)
'''
